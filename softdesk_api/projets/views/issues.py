...
class ProjectIssuesManager(APIView):
    ...
    def get(self, request, url_project_id=None, url_issues_id=None):
        ...
        if url_issues_id is None:
            ...
            try:
                ...
                serialized_issues = f"Aucune Issue n'existe à l'index {url_issues_id}"
    ...
    def put(self, request, url_project_id=None, url_issues_id=None):
        ...
        try:
            ...
            return Response(f"Aucune Issue n'existe à l'index {url_issues_id}")

        rdata = request.data
        new_assigned = rdata["assignee_user_id"]
        # If new assignee doesn't exist, abort & don't apply changes
        try:
            assignee_user_object = User.objects.get(
                id=rdata["assignee_user_id"])
        except ObjectDoesNotExist:
            return Response(f"Aucun user n'existe à l'index {new_assigned}")
